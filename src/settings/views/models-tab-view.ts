...
export class ModelsTabView extends AbstractTabView {
    ...
    render(containerEl: HTMLElement): void {
        ...
        pHead.createEl("th", { text: "ID / Name" });
        ...
        Object.keys(s.providers || {}).forEach(providerId => {
            ...
            if (["openai", "anthropic", "gemini", "deepseek", "ollama"].includes(providerId)) {
                ...
                // But we disable deletion to prevent breaking default configuration structure
        ...
        if (modelsList.length > 0) {
            ...
            emptyRow.createEl("td", { text: "暂无模型，点击上方按钮添加", attr: { colspan: "4", style: "text-align: center; color: var(--text-muted); font-style: italic; padding: 20px;" } });
        }
    ...
    private showEditProviderModal(providerId: string): void {
        ...
        const saveBtn = btns.createEl("button", { text: "保存", cls: "mod-cta" });
        saveBtn.onclick = async () => {
            this.settings.settings.providers[providerId].name = providerId; // Ensure name = ID
    ...
    private showAddModelModal(category: ModelCategory = MODEL_CATEGORIES.MULTIMODAL): void {
        ...
        save.onclick = async () => {
            ...
            const cfg: ModelConfig = {
                id: internalId,
                name: apiModelId, // Use API ID as name
                provider: providerId,
                model: apiModelId,
                enabled: true,
                category
    ...
    private showEditModelModal(modelId: string): void {
        ...
        save.onclick = async () => {
            if (!apiModelId) { new Notice("信息不能为空"); return; }
            this.settings.settings.models[modelId] = {
                ...m,
                provider: providerId,
                model: apiModelId,
                name: apiModelId // Sync name with ID
            };
            await this.settings.save();
            modal.close();
            if (this.lastContainerEl) this.render(this.lastContainerEl);
